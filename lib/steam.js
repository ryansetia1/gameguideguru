 ...
 * @param {boolean} [popup] When the flow runs in a `window.open` popup (mobile PWA
 *   fix — keeps Steam OpenID in a browser tab so the OS can't hand it to the native
 *   Steam app, and returns to the PWA via postMessage). Round-tripped as `p=1`.
 */
export function buildSteamLoginUrl(origin, state, intent = "link", popup = false) {
  const i = intent === "signin" ? "signin" : "link";
  const returnTo = `${origin}/api/steam/callback?s=${state}&i=${i}${popup ? "&p=1" : ""}`;
